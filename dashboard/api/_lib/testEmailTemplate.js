...
export function buildTestEmailSubject({ locationName }) {
  return `Test Email — ${locationName} — Future Insights`
}
...
export function buildTestEmail({ locationName, sentByName, sentAt }) {
  ...
  const html = `<!DOCTYPE html><html><body style="font-family:system-ui,sans-serif;max-width:560px;margin:0 auto;padding:24px;color:#1a1a1a;line-height:1.6">
    ...
    <p>This is a test email from the Future Insights Restaurant Contacts settings page, confirming email delivery is configured correctly for <strong>${safeLocation}</strong>.</p>
...
This is a test email from the Future Insights Restaurant Contacts settings page, confirming email delivery is configured correctly for ${locationName}.
