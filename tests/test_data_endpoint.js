// Regression tests for dashboard/api/data/[...path].js -- the authenticated
...
import handler from '../dashboard/api/data/[...path].js'
...
async function testEncodedTraversalRejected() {
  ...
  // how the request encoded it, and Vercel's own catch-all routing already
  // decodes segments before this handler ever sees them.
...
async function testSourceAndEnvFilesRejected() {
  await setDirectory()
  const token = await ownerToken()
  for (const segs of [['..', 'api', 'data', '[...path].js'], ['..', '..', '.env'], ['..', '..', '.git', 'config']]) {
...
async function testQueryStringIgnoredHarmlessly() {
  await setDirectory()
  const token = await ownerToken()
  // Vercel merges catch-all path segments and any ?query=string params into
  // the same req.query object -- confirms an extra query param neither
  // breaks the happy path nor helps smuggle anything past the allowlist.
  const req = { method: 'GET', query: { path: ['meta.json'], extra: 'ignored', foo: '../../reviews.db' }, headers: { cookie: `lta_session=${token}` } }
  const res = fakeRes()
  await handler(req, res)
  assert(res.statusCode === 200, `expected 200, got ${res.statusCode}`)
}
...
async function testEmptyCatchAllPathRejected() {
  ...
  for (const pathValue of [[], undefined, null]) {
    const req = { method: 'GET', query: { path: pathValue }, headers: { cookie: `lta_session=${token}` } }
    const res = fakeRes()
    await handler(req, res)
    assert(res.statusCode === 404, `empty catch-all path (${JSON.stringify(pathValue)}) must be rejected, got ${res.statusCode}`)
  }
}
