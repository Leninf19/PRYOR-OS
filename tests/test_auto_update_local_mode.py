...
async def test_db_path_validation_integration():
    with _Scratch(local_mode=True) as s:
        # Point db.DB_PATH somewhere that does NOT match BASE_DIR/dashboard/reviews.db.
        db.DB_PATH = s.base / "somewhere-else.db"
        _make_valid_db(db.DB_PATH)
        try:
            await auto_update.main()
            raise AssertionError("main() must refuse to run when db.DB_PATH doesn't match the expected path")
        except ValueError as e:
            assert "unexpected database path" in str(e)
...
def main():
    ...
    run("an unexpected db.DB_PATH is rejected before any mutation", test_db_path_validation_integration)
