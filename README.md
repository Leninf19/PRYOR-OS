...
| `MAIL_PROVIDER` / `MICROSOFT_TENANT_ID` / `MICROSOFT_CLIENT_ID` / `MICROSOFT_CLIENT_SECRET` / `MAIL_FROM_ADDRESS` / `MAIL_FROM_NAME` | `dashboard/api/_lib/emailSender.js` + `graphMailSender.js` (Microsoft Graph provider, staged migration off SMTP — see "Microsoft Graph migration" below) | Vercel project env vars, present but **inactive** until `MAIL_PROVIDER=graph` is set — any other/unset value keeps the existing SMTP path. |
...
| `BLOB_READ_WRITE_TOKEN` | `tenant_blob_store.py` (Multi-Tenant Phase 4F.1/4G — `provision_tenant.py`/`initial_sync.py`, and now `.github/workflows/tenant-lifecycle.yml`'s `provision`/`initial_sync` steps) | **GitHub Actions secret only.** Required before that workflow can provision or Initial-Sync a real second tenant — GitHub Actions has no Vercel OIDC identity, so it cannot use the same on-platform auth `dashboard/api/_lib/blobStore.js` relies on for the live dashboard; it needs this real, standalone read-write token for the SAME Vercel Blob store instead. Never set as a Vercel project env var (Vercel's own Blob access stays OIDC-only — see `blobStore.js`'s header comment for why re-adding it there is explicitly wrong). Not yet created as of Phase 4H.1 — this row is a reminder to add it before the tenant-lifecycle workflow is run against a real tenant, not a claim that it already exists. |
...
### Microsoft Graph migration (staged, inactive by default)

`sendReviewEmail()` (`dashboard/api/_lib/emailSender.js`) now supports a second delivery provider, Microsoft Graph's `sendMail` API, as a safer long-term replacement for password-based SMTP AUTH — selected via `MAIL_PROVIDER`:

- **`MAIL_PROVIDER` unset, or any value other than exactly `graph`** — unchanged SMTP behavior (this section's default state; deploying this code changes nothing until `MAIL_PROVIDER=graph` is set on purpose).
- **`MAIL_PROVIDER=graph`** — routes through `dashboard/api/_lib/graphMailSender.js` instead: OAuth2 client-credentials token from `https://login.microsoftonline.com/{MICROSOFT_TENANT_ID}/oauth2/v2.0/token` (scope `https://graph.microsoft.com/.default`), then `POST https://graph.microsoft.com/v1.0/users/{MAIL_FROM_ADDRESS}/sendMail`. No SMTP fallback on a Graph failure — once `graph` is explicitly selected, a failure is a failure, recorded truthfully as `failed` rather than silently retried through SMTP.
- The Entra app registration behind `MICROSOFT_CLIENT_ID`/`MICROSOFT_CLIENT_SECRET` is authorized (Exchange RBAC for Applications) to send only as `advertising@l3amigos.com` — `MAIL_FROM_ADDRESS` must match that mailbox exactly. This module makes no tenant-side changes and requests no additional permissions.
- `MAIL_FROM_NAME` sets the outgoing display name (Graph's `message.from.emailAddress.name`), the Graph-side equivalent of `SMTP_FROM_NAME`.
- To/CC/Reply-To/subject/HTML-body semantics are unchanged from the SMTP path; Graph's message body only supports a single content type per send, so the `text` fallback used by SMTP is not sent via Graph (HTML parity is the priority — see `sendGraphMail()`'s own comments).
- Tests: `tests/test_graph_mail_sender.js` (provider selection, token exchange, `sendMail` field mapping, sanitized-error behavior) — no real network calls, `globalThis.fetch` is mocked the same way `tests/test_google_oauth_auto_recovery.js` already does for Google's OAuth2 exchange.
- **To actually test Graph delivery**: set `MAIL_PROVIDER=graph` on a Vercel **Preview** environment (or a single Production env var flip once verified) — the `MICROSOFT_TENANT_ID`/`MICROSOFT_CLIENT_ID`/`MICROSOFT_CLIENT_SECRET`/`MAIL_FROM_ADDRESS`/`MAIL_FROM_NAME` values already exist in Vercel and need no further changes. Verify with **Settings → Restaurant Contacts → Send Test Email** against a test recipient before ever flipping Production. Flip `MAIL_PROVIDER` back to unset/`smtp` to roll back — no code change needed either direction.
